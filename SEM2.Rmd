...
The data was further cleaned by selecting cases from when countries had a total death toll of 5 and counting the weeks from that point, in an attempt to standardise the countries progression into the pandemic. Unfortunatley, joining and filtering results in only 123 observations.
...
```
Negative varianec is often a sign of an unidentifed model, however as the model should be recursive this is unlikely to be the case, more likely is that the structure is too different from what the data represents and there is not possible soltion, thus negative variances have to be introduced in order to enable fitting to occur, unfortunatley negative variances indicate a failed model [@rosseelStructuralEquationModeling].

```{r intial_paths}
# summary(cvd_sem, fit.measures=TRUE, standardized=TRUE)
semPaths(cvd_sem,  what = "stand", rotation = 2, layout = "tree")
```
There is no pracitcal value in evaluating the fit measures of this model, the paths can still be visualised indicating where the negative variances occur, many variances are ~50% of the total variation of the value, 0.5 due to the previous rescaling of distribtuions. Numerous tweaks were made to the model structure and associations in situ during development, however no solution that provided a useful outcome could be found.
